...
mysqli_query($conn,"insert into notice values('','$v','$sub','$details',now(),'$admin')");
		}
...
<form method="post">
	...
	<div class="row" style="margin-top:10px">
		<div class="col-sm-4">Select USN</div>
		<div class="col-sm-5">
		<select name="user[]" multiple="multiple" class="form-control">
			<?php 
	$sql=mysqli_query($conn,"select name,usn,email from users order by usn");
	while($r=mysqli_fetch_array($sql))
	{
		echo "<option value='".$r['usn']."'>".$r['usn']."</option>";
	}
			?>
		</select>
		</div>
